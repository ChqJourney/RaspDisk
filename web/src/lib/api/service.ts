...
class ApiService {
    ...
    async transferUploadFile(path:string,file: File): Promise<{ fileName: string }> {
        const formData = new FormData();
        formData.append('file', file);
        return this.request(`/transfer/upload?path=${path}`, {
            method: 'POST',
            body: formData
        });
    }
    async transferUploadLargeFile(path: string, file: File, setPrgress: (progress: number) => void, setIsPaused: (isPaused: boolean) => void): Promise<{ fileName: string }> {
        const CHUNK_SIZE = 3 * 1024 * 1024; // 5MB per chunk
        const totalSize = file.size;
        const totalChunks = Math.ceil(totalSize / CHUNK_SIZE);
        ...
        let isPaused = false;
        // 初始化上传
        const initResponse = await this.request<{ uploadId: string }>('/transfer/upload/large/init', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        const pauseUpload = async () => {
            isPaused = true;
            setIsPaused(true);
            await this.request(`/transfer/upload/large/pause/${uploadId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
        ...
        const stopUpload = async () => {
            isUploading = false;
            await this.request(`/transfer/upload/large/stop/${uploadId}`, {
                method: 'POST'
            });
            throw new Error('上传已停止');
        };
        ...
        const checkStatus = async () => {
            const status = await this.request<{
                ...
                status: string,
                fileName?: string
            }>(`/transfer/upload/large/status/${uploadId}`);
        ...
        const uploadChunk = async (chunkNumber: number): Promise<void> => {
            ...
            await this.request(`/transfer/upload/large/chunk/${uploadId}`, {
                method: 'POST',
                body: formData
            });
        };
     ...
     // 下载文件
     async transferDownloadFile(fileName: string, onProgress?: (progress: number) => void): Promise<Blob> {
        const url = `${this.baseURL}/transfer/download/${fileName}`;
        const headers = new Headers();
        const apiKey = localStorage.getItem('apiKey');
    ...
    async transferListFiles(path:string): Promise<Item[]> {
        return this.request(`/transfer/list?path=${path}`);
    }
}
